...
import { VendorTOSModal } from './vendor-tos-modal';
  ...
  const submitAuth = async (payload: Record<string, unknown>) => {
    ...
    try {
      ...
      if (data.requiresVerification) {
        ...
        showToast('Please verify your email before signing in.', { type: 'info' });
        return;
      }

      showToast(data?.message || (mode === 'signup' ? 'Account created' : 'Signed in'), { type: 'success' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authMethod === 'otp') {
      ...
      try {
        ...
        showToast('OTP sent to your email. Check your inbox.', { type: 'success' });
        router.push(`/otp?identifier=${encodeURIComponent(activeIdentifier)}&mode=${mode}`);
      } catch (err: any) {
    ...
    <div className="w-full rounded-[2rem] border border-black/10 bg-white/90 p-8 shadow-soft backdrop-blur-sm">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-[26ch] text-xs leading-5 text-brand-gray">Use email for account access and OTP; add a phone number for sales follow-up.</p>
        </div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 rounded-2xl bg-black/5 p-1">
        <button
          ...
          className={`rounded-[0.9rem] px-3 py-2 text-sm font-semibold transition ${authMethod === 'otp' ? 'bg-white text-brand-black shadow-sm' : 'text-brand-gray hover:text-brand-black'}`}
        ...
        <button
          ...
          className={`rounded-[0.9rem] px-3 py-2 text-sm font-semibold transition ${authMethod === 'password' ? 'bg-white text-brand-black shadow-sm' : 'text-brand-gray hover:text-brand-black'}`}
        ...
        onAccept={async () => {
          ...
          if (authMethod === 'otp') {
            ...
            try {
              ...
              router.push(`/otp?identifier=${encodeURIComponent(activeIdentifier)}&mode=signup`);
            } catch (err: any) {
              setError(err.message || 'Failed to send OTP');
            } finally {
              setLoading(false);
